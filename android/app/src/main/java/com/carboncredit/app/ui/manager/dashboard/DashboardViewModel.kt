    ...
    fun loadDashboard() {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                android.util.Log.d("CarbonCreditApp", "Fetched today credits size: ${todayCredits.size}, credits: $todayCredits")
    ...
    private fun startRealtimeUpdates(facilityId: String) {
        viewModelScope.launch {
            try {
                ...
                launch {
                    try {
                        creditRepository.subscribeCreditChannel(facilityId)
                        creditRepository.subscribeToCredits(facilityId).collect {
                            loadDashboard() // Reload all credits when a new one is issued
                        }
                    } catch (e: Exception) {
                        android.util.Log.e("CarbonCreditApp", "Credit realtime error: ${e.message}")
                    }
                }
            } catch (e: Exception) { 
                android.util.Log.e("CarbonCreditApp", "Realtime error: ${e.message}")
            }
        }
    }
