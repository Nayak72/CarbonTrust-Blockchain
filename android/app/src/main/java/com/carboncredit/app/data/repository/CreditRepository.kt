...
class CreditRepository @Inject constructor(
    ...
    fun subscribeToCredits(facilityId: String): Flow<PostgresAction.Insert> {
        val channel = supabase.realtime.channel("credits-$facilityId")
    ...
    suspend fun subscribeCreditChannel(facilityId: String) {
        val channel = supabase.realtime.channel("credits-$facilityId")
        channel.subscribe()
    }
}
