...
class SimulatorStartRequest(BaseModel):
    ...
    # Physics-informed simulator parameters
    facility_type: Optional[str] = "manufacturing"
    baseline_ppm: Optional[float] = 600.0
    inject_anomaly_rate: Optional[float] = 0.02
...
async def start_simulator(
    ...
    """
    Start a physics-informed sensor simulator for a specific facility.
    ...
    Optional parameters:
      - facility_type: drives the baseline emission curve (default: "manufacturing")
      - baseline_ppm: facility-specific mean CO₂ ppm (default: 600.0)
      - inject_anomaly_rate: probability of injecting an anomaly per tick (default: 0.02)
    """
    ...
    success = simulator_manager.start(
        ...
        facility_type=body.facility_type or "manufacturing",
        baseline_ppm=body.baseline_ppm or 600.0,
        inject_anomaly_rate=body.inject_anomaly_rate or 0.02,
    )
    ...
    return {
        "status": "started",
        "message": (
            f"Simulator running for facility {body.facility_id} — "
            f"readings every {body.interval_seconds}s, "
            f"type={body.facility_type}, baseline={body.baseline_ppm} ppm"
        )
    }
